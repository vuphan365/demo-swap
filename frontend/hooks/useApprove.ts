...
export const useApprove = () => {
    const [approveStatus, setApproveStatus] = useState<ApproveStatus>(
    ...
    const onCheckTokenApprove = useCallback(
        debounce(async ({ token, amount }: ApproveParams) => {
            try {
    ...
    const onApproveToken = useCallback(
        async (token: Token) => {
            ...
            try {
                ...
                const hash = await web3Provider
                    .getSigner(0)
                    .sendUncheckedTransaction(unsignedTransaction)
                // @ts-ignore
